...
window.toast = {
  success: m => window._toast?.show(m, 'success'),
  error: m => window._toast?.show(m, 'error'),
  info: m => window._toast?.show(m, 'info')
};
...
function initSocket(auth) {
  ...
  _socket.on('new_booking_request', data => {
    toast.info(`New booking from ${data.passenger}`);
  });
...
//   }
//   async api(path, options = {}) {
//     const res = await fetch(path, { headers: { ...this.jsonHeaders, ...(options.headers || {}) }, ...options });
//     const data = await res.json().catch(() => ({}));
...
// function initSocket(auth) {
//   if (_socket || typeof io === 'undefined') return;
//   _socket = io({ auth: { token: auth.token }, transports: ['websocket', 'polling'] });
